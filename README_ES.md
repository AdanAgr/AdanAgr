<p align="right">
  <a href="README.md">EN English</a> | ES <b>Español</b>
</p>

[![Welcome](https://readme-typing-svg.demolab.com?font=Fira+Code\&pause=1000\&color=8DCAF9\&width=435\&lines=Viaje+Antes+Que+Destino)](https://git.io/typing-svg)

<p align="center">
  <img src="assets/banner_es.png" alt="Adán González Rodríguez - Ingeniero en Inteligencia Artificial" width="100%" />
</p>

## 🧙‍♂️ Sobre mí

* 🎓 Graduado en **Inteligencia Artificial** por la **Universidad de Vigo (UVigo)**.
* 📚 Actualmente estudiando un **Máster en Big Data** en la **Universidad de Santiago de Compostela (USC)**.

---

## 🎯 Áreas de interés

`Inteligencia Artificial` · `Machine Learning` · `Deep Learning` · `Big Data` · `Ingeniería de Datos`

---

## 🛠️ Tecnologías

<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=python,java,sklearn,vscode,git,github" alt="Tecnologías" />
  </a>
</p>

---

## 🤝 Contacto

<p align="center">
  <a href="https://www.linkedin.com/in/adanagr">
    <img src="https://skillicons.dev/icons?i=linkedin" alt="LinkedIn" width="40" height="40"/>
  </a>
  &nbsp;&nbsp;
  <a href="mailto:adan.rozaguez@gmail.com">
    <img src="https://skillicons.dev/icons?i=gmail" alt="Gmail" width="40" height="40"/>
  </a>
</p>
